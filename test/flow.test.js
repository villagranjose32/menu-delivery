...
test('registro, aprobación, menú público e imagen', async () => {
  ...
  const geocoder = http.createServer((req, res) => { res.setHeader('Content-Type', 'application/json'); if(req.url.startsWith('/route/')) res.end(JSON.stringify({code:'Ok',routes:[{distance:4200,duration:720}]})); else if(req.url.startsWith('/v1/geocode/autocomplete')) { lastAutocompleteRequest=req.url;res.end(JSON.stringify({results:[{formatted:'Av. Roca 100, Centro, Santiago del Estero, Argentina',street:'Av. Roca',housenumber:'100',suburb:'Centro',lat:-27.8,lon:-64.27}]})); } else if(req.url.startsWith('/reverse')) res.end(JSON.stringify({display_name:'Av. Roca 100, Centro, Santiago del Estero, Argentina',lat:'-27.8',lon:'-64.27',address:{road:'Av. Roca',house_number:'100',suburb:'Centro'}})); else { lastGeocodeRequest=req.url; res.end(JSON.stringify([{ lat: '-27.7951', lon: '-64.2615', display_name: 'Av. Belgrano 1240, Centro, Santiago del Estero', address:{road:'Av. Belgrano',house_number:'1240',suburb:'Centro',country_code:'ar',state:'Santiago del Estero'} }])); } });
  ...
  try {
    ...
    let reverse = await request('/api/reverse-geocode?lat=-27.8&lng=-64.27'); assert.equal(reverse.status,200);assert.equal(reverse.data.displayName,'Av. Roca 100, Centro');
    ...
    let suggestions = await request('/api/address-suggestions?q=Av.%20Roca&near=-27.8,-64.26'); assert.equal(suggestions.status,200);assert.equal(suggestions.data.suggestions[0].label,'Av. Roca 100, Centro');assert.match(lastAutocompleteRequest,/25000/);
