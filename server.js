...
function compactAddress(address, fallback = '') { const a=address||{}, street=safeText(a.road||a.street||a.pedestrian||a.address_line1||a.name,120), number=safeText(a.house_number||a.housenumber,20), neighborhood=safeText(a.suburb||a.neighbourhood||a.neighborhood||a.quarter||a.city_district||a.district,100); let first=street; if(number&&street&&!street.toLowerCase().includes(number.toLowerCase()))first=`${street} ${number}`; const compact=[first,neighborhood].filter(Boolean).join(', '); return compact||safeText(fallback,300).split(',').slice(0,2).join(',').trim(); }
...
async function handler(req, res) {
  ...
  if (method === 'GET' && url.pathname === '/api/geocode') {
    ...
    const key = crypto.createHash('sha256').update(`v2|${contextualQuery.toLowerCase()}|${country}|${near?`${near.lat.toFixed(2)},${near.lng.toFixed(2)}`:''}`).digest('hex');
    ...
    try {
      ...
      const result = { lat: Number(results[0].lat), lng: Number(results[0].lon), displayName: compactAddress(results[0].address,results[0].display_name), countryCode:safeText(results[0].address?.country_code,2).toLowerCase(), state:safeText(results[0].address?.state||results[0].address?.province,100) };
  ...
  if (method === 'GET' && url.pathname === '/api/reverse-geocode') {
    ...
    const key=`reverse:v2:${lat.toFixed(5)},${lng.toFixed(5)}`;if(db.geocode[key])return json(res,200,db.geocode[key]);if(Date.now()-lastGeocodeAt<1100)return error(res,429,'Esperá un segundo antes de buscar otra ubicación.');lastGeocodeAt=Date.now();
